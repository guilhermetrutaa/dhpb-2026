...
export default function ModalQuestionarioIndividual({ authUser, userData, edicaoId, onComplete, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setEnviando(true)
    try {
      await setDoc(doc(db, 'users', authUser.uid, 'questionarios', edicaoId), {
        ...formData,
        tipo,
      ...
      <div className="flex items-start justify-center min-h-full px-4 py-8">
        <div className="relative w-full max-w-3xl bg-white">
          <div className="bg-[#82181A] text-white px-8 py-6 relative">
            ...
            <p className="text-white/80 text-sm mt-1">Preencha seus dados para participar do DHPB</p>
            {onClose && (
