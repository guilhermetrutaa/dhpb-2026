...
function QuestoesForm() {
  ...
  useEffect(() => {
    ...
    if (!faseId || !edicaoId) {
      setErroTarefa('Faltam faseId e edicaoId. Abra pelo dashboard.')
      setCarregando(false)
      return
    }
  ...
  const handleSalvarTarefa = async () => {
    ...
    const faseRef = doc(db, 'edicoes', edicaoId, 'fases', faseId)
    try {
      try {
        await updateDoc(faseRef, payload)
      } catch (err) {
        const notFound = err?.code === 'not-found' || /No document to update/i.test(err?.message || '')
        if (!notFound) throw err
        await setDoc(faseRef, payload, { merge: true })
      }
      setMsgTarefa('Tarefa salva.')
    } catch (err) {
      setErroTarefa(err?.message || 'Não foi possível salvar a tarefa.')
    } finally {
        ...
        <main className='max-w-5xl mx-auto px-4 py-8 space-y-8'>
          <div className='bg-white rounded-xl shadow-md p-6'>
            ...
            <button
              ...
              className="bg-[#82181A] text-white text-sm font-semibold px-6 py-2 rounded-lg hover:bg-[#631214] disabled:opacity-50 transition-colors cursor-pointer"
            >
              {salvandoTarefa ? 'Salvando...' : 'Salvar Tarefa'}
            </button>
            {msgTarefa && <p className="text-green-700 text-sm mt-3">{msgTarefa}</p>}
            {erroTarefa && <p className="text-red-600 text-sm mt-3">{erroTarefa}</p>}
          </div>
