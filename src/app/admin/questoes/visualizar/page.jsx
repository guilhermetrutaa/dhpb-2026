    ...
    <header className="flex flex-col items-center justify-around gap-6 px-4 pb-5 pt-5 lg:flex-row">
      ...
      <nav>
        <ul className="flex flex-wrap justify-center gap-4 text-sm md:gap-6 md:text-base">
          <li className="transition-colors hover:text-[#82181A] hover:underline"><a href="/home">Home</a></li>
          <li className="transition-colors hover:text-[#82181A] hover:underline"><a href="/sala-de-equipe">Sala de Prova</a></li>
      ...
      <Link
        ...
        className="whitespace-nowrap border-[3px] border-[#82181A] px-6 py-2 font-medium text-[#82181A] transition-colors hover:bg-[#82181A] hover:text-white"
      >
        Voltar ao painel
      </Link>
    </header>
...
function VisualizarContent() {
  ...
  const voltarHref = `/admin/questoes?faseId=${faseId || ''}&edicaoId=${edicaoId || ''}`
  ...
  useEffect(() => {
    ...
    const carregar = async () => {
      if (!faseId || !edicaoId || !questaoId) {
        setErro('Parâmetros inválidos para visualização.')
        setCarregando(false)
        return
      }
      try {
    ...
    carregar()
  }, [autenticado, faseId, edicaoId, questaoId])
  ...
  const prevId = currentIdx > 0 ? allQuestaoIds[currentIdx - 1] : ''
  const nextId = currentIdx >= 0 && currentIdx < allQuestaoIds.length - 1 ? allQuestaoIds[currentIdx + 1] : ''
  const prevHref = prevId ? `/admin/questoes/visualizar?questaoId=${prevId}&faseId=${faseId}&edicaoId=${edicaoId}` : ''
  const nextHref = nextId ? `/admin/questoes/visualizar?questaoId=${nextId}&faseId=${faseId}&edicaoId=${edicaoId}` : ''
  ...
  if (erro) {
    return (
      <div className={`${poppins.className} flex min-h-screen flex-col items-center justify-center bg-gray-100 p-4`}>
        ...
        <Link href={voltarHref} className="text-blue-600 hover:underline">
          Voltar
        </Link>
      </div>
    )
      ...
      <div className="min-h-screen w-full bg-white text-black">
        ...
        <section className="pb-20">
          ...
          <div className="mx-auto mt-12 max-w-screen px-4">
            <div>
              {questao.alternativas?.map((alt, index) => {
                ...
                return (
                  <div
                    key={alt.letra}
                    className={`grid w-full grid-cols-[40px_1fr] gap-4 px-4 py-10 text-left text-[0.98rem] leading-snug text-[#333] ${rowBase}`}
                  >
            ...
            <div className="mx-auto mt-12 max-w-md rounded-lg border border-[#E8D48B] bg-[#FFF9E6] p-4 text-center text-sm font-medium text-[#7A6A2F]">
              Modo prévia do administrador — os alunos verão os botões de resposta aqui quando a fase abrir.
            </div>
