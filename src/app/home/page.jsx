...
const Page = () => {
  ...
  const cidades = [
    ...
    { src: '/cidades-avatar/cabedelo.svg', nome: 'Cabedelo' },
    { src: '/cidades-avatar/campina grande.svg', nome: 'Campina Grande' },
    { src: '/cidades-avatar/esperanca.svg', nome: 'Esperança' },
    { src: '/cidades-avatar/patos.svg', nome: 'Patos' },
    { src: '/cidades-avatar/picui.svg', nome: 'Picuí' },
    { src: '/cidades-avatar/sape.svg', nome: 'Sapé' },
    { src: '/cidades-avatar/sossego.svg', nome: 'Sossego' },
  ]
  const avatarAtual = userData?.avatar || '/avatar.svg'
  ...
  return (
    <div className={poppins.className}>
      <div className='w-full min-h-screen bg-[#fff] text-[#000]'>
        ...
        <header className='flex flex-col lg:flex-row justify-around items-center pt-5 gap-6 px-4'>
          ...
          <nav>
            <ul className='flex flex-wrap justify-center gap-4 md:gap-6 text-sm md:text-base'>
              ...
              <li className='hover:text-[#82181A] hover:underline transition-colors'><a href="/provas-antigas">Provas Antigas</a></li>
              <li className='hover:text-[#82181A] hover:underline transition-colors'><a href="/biblioteca">Biblioteca</a></li>
        ...
        <main className='pt-16 md:pt-30 px-4'>
            <div className='max-w-7xl mx-auto'>
                <div className='text-center'>
                    <h1 className='text-[1.2rem] md:text-[1.5rem] font-bold text-[#82181A] break-words'>
                      Bem vindo(a), {nome}
                    </h1>
                </div>
                ...
                <div className='flex flex-col items-center pt-5 relative'>
                    <Image
                      ...
                      height={120}
                      alt="Avatar"
                      className='rounded-full border-[#82181A] border-3 h-auto'
                    />
                    <button onClick={() => setShowAvatares(!showAvatares)} className='mt-2 text-xs text-[#82181A] font-semibold hover:underline cursor-pointer'>
                      Trocar avatar
                    </button>

                    {showAvatares && (
                      <div className='absolute top-32 z-10 bg-white rounded-2xl shadow-xl border border-neutral-200 p-4 min-w-[280px]'>
                        <p className='text-xs font-semibold text-neutral-500 mb-3 text-center'>Escolha seu avatar</p>
                        <div className='flex gap-1 mb-3 bg-neutral-100 rounded-lg p-1'>
                          <button onClick={() => setAbaAvatar('avatares')} className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-colors cursor-pointer ${abaAvatar === 'avatares' ? 'bg-white text-[#82181A] shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}>Avatares</button>
                          <button onClick={() => setAbaAvatar('cidades')} className={`flex-1 py-1.5 text-xs font-semibold rounded-md transition-colors cursor-pointer ${abaAvatar === 'cidades' ? 'bg-white text-[#82181A] shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}>Cidades Premiadas 2025</button>
                        </div>
                        <div className='flex flex-wrap gap-3 justify-center'>
                          {(abaAvatar === 'avatares' ? avatares : cidades).map((item) => {
                            const src = typeof item === 'string' ? item : item.src
                            const nome = typeof item === 'string' ? '' : item.nome
                            const isSelected = avatarAtual === src
                            return (
                              <button key={src} onClick={() => trocarAvatar(src)} className='cursor-pointer hover:scale-110 transition-transform flex flex-col items-center gap-1'>
                                <Image src={src} width={60} height={60} alt={nome || 'Avatar'} className={`rounded-full border-3 ${isSelected ? 'border-[#82181A]' : 'border-transparent'}`} />
                                {nome && <span className='text-[10px] text-neutral-500 whitespace-nowrap'>{nome}</span>}
                              </button>
                            )
                          })}
                        </div>
                ...
                <div className='flex justify-center pt-7'>
                    <p className='text-[1rem] md:text-[1.1rem] text-[#313131] font-medium w-full max-w-[55rem] text-center leading-relaxed'>
                      Oi {nome}! Receba nossas boas-vindas à sua área pessoal no Desafio em História da Paraíba. Aqui, informações atualizadas sobre sua participação em nossos programas.
                    </p>
                </div>
                ...
                <div className='pt-10'>
                    ...
                    <div className='flex flex-wrap justify-center gap-4 pt-5 pb-10'>
                      {edicoes.length === 0 ? (
                        <p className='text-neutral-500'>Nenhuma edição disponível no momento.</p>
                      ) : (
                        edicoes.map((ed) => (
                          <button
                            key={ed.id}
                            onClick={() => handleEdicaoClick(ed.id)}
                            className='bg-[#82181A] w-full max-w-[14rem] h-[4rem] text-[#fff] font-semibold cursor-pointer rounded-[5px] active:scale-95 transition-transform hover:bg-[#631214]'
                          >
                            {ed.nome}
                          </button>
                        ))
